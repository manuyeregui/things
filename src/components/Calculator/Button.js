...
export default function Button({ value, theme }) {
  ...
  const signClick = () => {
    setCalc({
      ...
      res:
        !calc.res && calc.num
          ? calc.num
          : calculate(calc.res, calc.num, calc.sign),
      num: 0,
    });
  };
  ...
  return (
    <div
      className={`cursor-pointer select-none text-3xl py-6 px-12 hover:bg-gray-500/5 active:bg-gray-500/10 rounded text-center ${
        value === "=" && "col-span-2"
      }`}
